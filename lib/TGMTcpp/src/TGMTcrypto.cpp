...
std::string TGMTcrypto::EncryptStringAES(std::string value, std::string password)
{
...
std::string TGMTcrypto::DecryptStringAES(std::string value, std::string password)
{
...
bool TGMTcrypto::EncryptFileAES(std::string infile, std::string outfile, std::string password)
{
	if (!TGMTfile::FileExist(infile))
	{
		PrintError("File %s does not exist", infile.c_str());
		return false;
	}

	std::string content = TGMTfile::ReadAllText(infile);
	if (content.length() == 0)
		return false;

	std::string encryptContent = EncryptStringAES(content, password);
	if (encryptContent.length() == 0)
		return false;

	return TGMTfile::WriteToFile(outfile, encryptContent, false);
}

///////////////////////////////////////////////////////////////////////////////////////////////////

bool TGMTcrypto::DecryptFileAES(std::string infile, std::string outfile, std::string password)
{
	if (!TGMTfile::FileExist(infile))
	{
		PrintError("File %s does not exist", infile.c_str());
	...
	std::string decryptContent = DecryptStringAES(content, password);
	if (decryptContent.length() == 0)
		return false;

	return TGMTfile::WriteToFile(outfile, decryptContent, false);
}