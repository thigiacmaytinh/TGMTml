...
cv::Mat TGMTtransform::CropImage(cv::Mat matInput, cv::RotatedRect rrect)
{
	...
	cv::Mat matMatrix = cv::getRotationMatrix2D(rrect.center, angle, 1.0);
	// perform the affine transformation
	cv::warpAffine(matInput, rotated, matMatrix, matInput.size(), cv::INTER_CUBIC);
...
cv::Mat TGMTtransform::ResizeByScreen(cv::Mat matInput, int padding)
{
	int screenW, screenH;
	TGMThardware::GetScreenResolution(screenW, screenH);

	if (screenW < matInput.cols - padding)
	{
		return TGMTtransform::ResizeByWidth(matInput, screenW - padding);
	}
	else if (screenH < matInput.rows - padding)
	{
		return TGMTtransform::ResizeByHeight(matInput, screenH - padding);
	}
	else
	{
		return matInput;
